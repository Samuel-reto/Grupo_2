 ...
 * Description: Sistema completo citas médicas - MiSalud@SCS con registro
 * Version: 1.1
 * Author: Raquel ASIR
 */
...
class Health2You {
    ...
    add_shortcode('h2y_registro_medico', [$this, 'registro_medico_shortcode']); // ← NUEVA LÍNEA
...
public function registro_medico_shortcode() {
    ...
    public function paciente_shortcode() {
        if (!isset($_SESSION['h2y_tipo']) || $_SESSION['h2y_tipo'] !== 'paciente') {
            return '<p>Acceso denegado. <a href="' . home_url('/login-citas/') . '">Iniciar sesión</a></p>';
        }
        ob_start();
